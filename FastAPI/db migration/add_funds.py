...
def add_funds():
    ...
    with engine.sync_session() as db:
        ...
        for record in db.scalars(
            select(models.Record).options(selectinload(models.Record.created_by))
        ).all():
            user_id = record.created_by

            if user_id:
                user_id = user_id.old_id
                fund = db.scalar(
                    select(models.Fund).where(models.Fund.old_id == user_props.get(user_id))
                )
                record.fund = fund
                db.commit()
