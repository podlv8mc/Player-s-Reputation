...
class User(SQLAlchemyBaseUserTable[int], Base):
    ...
    email: Mapped[str] = mapped_column(String(256), server_default="-")
    discord: Mapped[str] = mapped_column(String(256), server_default="-")
...
class Fund(Base):
    ...
    email: Mapped[str] = mapped_column(String(), server_default="-")
    discord: Mapped[str] = mapped_column(String(), server_default="-")
    ...
    old_id: Mapped[str] = mapped_column(String(), server_default="-")
...
class Record(Base):
    ...
    id = Column(Integer, primary_key=True)
    first_name = Column(String(64), server_default="-")
    last_name = Column(String(64), server_default="-")
    middlename = Column(String(64), server_default="-")
    nicknames = relationship("Nickname", back_populates="record")
    gipsyteam = Column(Text(), server_default="-")
    pokerstrategy = Column(Text(), server_default="-")
    description = Column(Text(), server_default="-")
    amount = Column(Text(), server_default="-")
    google = Column(Text(), server_default="-")
    mail = Column(Text(), server_default="-")
    vk = Column(Text(), server_default="-")
    facebook = Column(Text(), server_default="-")
    blog = Column(Text(), server_default="-")
    instagram = Column(Text(), server_default="-")
    forum = Column(Text(), server_default="-")
    neteller = Column(Text(), server_default="-")
    skrill = Column(Text(), server_default="-")
    ecopayz = Column(Text(), server_default="-")
    old = Column(Boolean(), default=False)
    fundName = Column(Text(), server_default="-")
    fund = relationship(Fund, back_populates="records")
    fund_id = Column(Integer, ForeignKey("funds.id"))
    nicknameOld = Column(Text(), server_default="-")
    comments = Column(Text(), server_default="-")
    country = Column(Text(), server_default="-")
    town = Column(Text(), server_default="-")
    address = Column(Text(), server_default="-")
    created_by_id = Column(Integer, ForeignKey("users.id"), nullable=True)
    created_by = relationship("User")
    created_at = Column(DateTime(timezone=True))
    webmoney_id = Column(Text(), server_default="-")
    wallets = Column(Text(), server_default="-")
...
class RecordHistory(Base):
    ...
    old_id = Column(String(), server_default="-")
